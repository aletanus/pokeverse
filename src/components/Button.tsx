...
export default function Button(props: ButtonProps) {
  ...
  return (
    <button onClick={props.onClick} disabled={props.disabled} type={props.type} className={`border rounded flex items-center  ${size()} ${style()} ${props.details} ${props.disabled && "cursor-not-allowed"} transition-all duration-300 ease-in-out`} style={{ width: props.width, }} hidden={props.hiddenOnSmallScreens}>
      {props.children}
    </button>
  );
}
